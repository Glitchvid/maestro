	...
	local q = mysql:Select("maestro_bans")
		...
		q:Callback(function(res, status)
			if type(res) == "table" then
				...
				maestro.chatconsole(caller, maestro.colors.white, "Banlog for (", maestro.colors.blue, id, maestro.colors.white, "):")
				for i = 1, #res do
					local ban = res[i]
					local col = tonumber(ban["until"]) > os.time() and maestro.colors.orange or maestro.colors.blue
					local len = ban["until"] == 0 and "permanent" or maestro.time(ban["until"] - ban.when)
					maestro.chatconsole(caller, col, "\t", os.date("%x - ", ban.when), "#", ban.id, ", ", ban.admin, " (", util.SteamIDFrom64(ban.adminid), "): ", maestro.colors.white, ban.reason, col, " Length: ", maestro.colors.white, len)
				end
			end
		end)
	q:Execute()
